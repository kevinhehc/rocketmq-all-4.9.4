...
public class ReplyMessageProcessor extends AbstractSendMessageProcessor implements NettyRequestProcessor {
     ...
     *         如果是批量消息或者轻量（压缩）消息，那么先解析为SendMessageRequestHeaderV2，
     *         然后转换为SendMessageRequestHeader，否则直接解析为SendMessageRequestHeader；
     *      2. 我们之前讲过，在Producer发送消息的时候可能会使用轻量级消息头SendMessageRequestHeaderV2，
     *         SendMessageRequestHeaderV2相比于SendMessageRequestHeader，其field 全为 a,b,c,d 等短变量名，
     *         可以加快FastJson反序列化过程，提升传输效率。；
     * @param request
     * @return
