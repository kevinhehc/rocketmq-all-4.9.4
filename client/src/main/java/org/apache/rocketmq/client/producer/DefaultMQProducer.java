 ...
 */
// 这个类是打算发送消息的应用程序的入口点。调整暴露getter/setter方法的字段是可以的，但请记住，在大多数情况下，它们都应该可以开箱即用。
// 这个类聚合了各种发送方法，以便将消息传递给代理。它们都有优点和缺点;在实际编码之前，你最好了解它们的优缺点。
// 线程安全:在配置和启动进程后，该类可以被认为是线程安全的，可以在多个线程上下文中使用。
public class DefaultMQProducer extends ClientConfig implements MQProducer {
     ...
     */
    // 生产者组在概念上聚合了具有完全相同角色的所有生产者实例，当涉及事务消息时，这一点特别重要。
    // 对于非事务性消息，只要它在每个进程中是唯一的就没关系。更多讨论请参见核心概念。
     ...
     */
    // 在同步模式下声明发送失败之前在内部重试执行的最大次数。
    // 这可能会导致消息重复，这需要应用程序开发人员解决。
     ...
     * This may potentially cause message duplication which is up to application developers to resolve.
     */
    // 在异步模式下声明发送失败之前在内部重试执行的最大次数。这可能会导致消息重复，这需要应用程序开发人员解决。
     ...
     */
    // 启动这个生产者实例。很多内部的初始化过程都是为了让这个实例做好准备，因此，在发送或查询消息之前必须调用这个方法。
    @Override
     ...
     */
    // 以同步方式发送消息。此方法仅在发送过程完全完成时返回。警告:此方法具有内部重试机制，
    // 即内部实现将在声称失败之前重试retryTimesWhenSendFailed次数。
    // 因此，可能会有多个消息传递给broker。解决潜在的重复问题取决于应用程序开发人员。
    @Override
     ...
     * @throws InterruptedException if the sending thread is interrupted.
     */
    // 与send(Message)也是一样，另外还指定了发送超时时间。
    @Override
     ...
     * @throws InterruptedException if the sending thread is interrupted.
     */
    // 异步发送消息给broker。这个方法会立即返回。一旦发送完成，sendCallback就会被执行。
    // 与send(Message)类似，内部实现在声称发送失败之前可能会重试retryTimesWhenSendAsyncFailed次数，
    // 这可能会产生消息重复，而应用程序开发人员是解决这个潜在问题的人。
    @Override
     ...
     * @throws InterruptedException if the sending thread is interrupted.
     */
    // 与send(Message, SendCallback)一样，同时也指定了发送超时时间。
    @Override
     ...
     * @throws InterruptedException if the sending thread is interrupted.
     */
    // 与UDP类似，这个方法不会在返回之前等待broker的确认。显然，它有最大的吞吐量，但有可能丢失消息。
    @Override
     ...
     * @throws InterruptedException if the sending thread is interrupted.
     */
    // 与send(Message)一样，同时目标消息队列和发送超时指定。
    @Override
     ...
     * @throws InterruptedException if the sending thread is interrupted.
     */
    // 与 send(Message, SendCallback) 一样，同时指定目标消息队列。
    @Override
     ...
     * @throws InterruptedException if the sending thread is interrupted.
     */
    // 与 send(Message, SendCallback)一样， 同时指定目标消息队列和发送超时。
    @Override
     ...
     * @throws InterruptedException if the sending thread is interrupted.
     */
    // 与sendOneway(Message)一样，同时指定目标消息队列。
    @Override
     ...
     * @throws InterruptedException if the sending thread is interrupted.
     */
    //与 send(Message, MessageQueueSelector, Object)一样，同时指定了超时时间
    @Override
     ...
     * @throws InterruptedException if the sending thread is interrupted.
     */
    // 与send(Message, SendCallback) 一样，同时指定了队列选择器
    @Override
     ...
     * @throws InterruptedException if the sending thread is interrupted.
     */
    // 与  send(Message, MessageQueueSelector, Object, SendCallback) 一样，同时指定了超时时间
    @Override
     ...
     */
    // 以同步方式发送请求消息。此方法仅在使用者使用请求消息并应答消息时返回。
    // 警告:此方法具有内部重试机制，即内部实现将在声称失败之前重试retryTimesWhenSendFailed次数。
    // 因此，可能会有多个消息传递给broker。解决潜在的重复问题取决于应用程序开发人员。
    @Override
     ...
     * @throws MQBrokerException if there is any broker error.
     */
    // 异步请求。这个方法会立即返回。收到回复消息后，将执行requestCallback。
    // 与request(Message, long)类似，内部实现在声称发送失败之前可能会重试retryTimesWhenSendAsyncFailed次数，
    // 这可能会产生消息重复，应用程序开发人员是解决这个潜在问题的人。
    @Override
     ...
     * @throws RequestTimeoutException if request timeout.
     */
    // 与 request(Message, long) 一样，同时指定了消息队列选择器
    @Override
     ...
     * @throws MQBrokerException if there is any broker error.
     */
    // 与 request(Message, RequestCallback, long) 一样，同时指定了消息队列选择器
    @Override
     ...
     * @throws RequestTimeoutException if request timeout.
     */
    // 与 request(Message, long)一样，另外指定了目标消息队列。
    @Override
     ...
     * @throws MQBrokerException if there is any broker error.
     */
    // 与  request(Message, RequestCallback, long) 一样，同时指定消息队列
    @Override
     ...
     * @throws InterruptedException if the sending thread is interrupted.
     */
    // 与  sendOneway(Message) 一样，同时指定队列选择器
    @Override
     ...
     */
    // 设置一个用于执行回调方法的执行器。如果没有设置执行器，则netttyremotingclient。将使用publicExecutor。
     ...
     */
    // 设置一个用于异步发送的执行器。如果没有设置执行器，则将使用DefaultMQProducerImpl.defaultAsyncSenderExecutor。
