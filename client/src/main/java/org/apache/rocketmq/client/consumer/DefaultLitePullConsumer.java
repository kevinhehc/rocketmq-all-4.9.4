...
public class DefaultLitePullConsumer extends ClientConfig implements LitePullConsumer {
     ...
     * consumer groups.<br/>
     * 属于同一个消费者组的消费者共享一个组id。然后，组中的消费者通过建立每个队列只被组中的单个消费者消费的机制，尽可能公平地在自己之间划分主题。
     * 如果所有消费者都来自同一组，则它将充当传统的消息队列。每条消息只能被组中的一个consumer使用。当存在多个消费者群体时，
     * 数据消费模型的流程与传统的发布-订阅模型保持一致。消息被广播到所有消费者组。
     */
    private String consumerGroup;

    /**
     * Long polling mode, the Consumer connection max suspend time, it is not recommended to modify
     * 长轮询模式，消费者连接最大挂起时间，不建议修改
     */
    private long brokerSuspendMaxTimeMillis = 1000 * 20;

    /**
     * Long polling mode, the Consumer connection timeout(must greater than brokerSuspendMaxTimeMillis), it is not
     * recommended to modify<br/>
     * 长轮询模式，消费者连接超时(必须大于brokerSuspendMaxTimeMillis)，不建议修改
     */
    ...
    /**
     * Consumption pattern,default is clustering<br/>
     * 消费模式，默认是集群
     */
     ...
     * Consider the {@code pullBatchSize}, the instantaneous value may exceed the limit<br/>
     * 消费请求的流量控制阈值，默认情况下，每个消费者最多缓存10000个消费请求。考虑pullBatchSize，瞬时值可能超过限制
     */
     ...
     * the {@code pullBatchSize}, the instantaneous value may exceed the limit<br/>
